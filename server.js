...
app.get('/goods/:g_code', (req, res) => {
  ...
  connectionKdt.query(
    ...
    (err, results) => {
      ...
      if (results.length === 0) {
        return res.status(404).json({ error: '해당 상품이 없습니다.' });
      }
      res.json(results[0]);
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: '도서 등록 실패' });
      }
      res.json({ success: true, insertId: result.insertId });
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, results) => {
      if (err) {
        ...
        return res.status(500).json({ error: '도서 조회 실패' });
      }
      if (results.length === 0) {
        return res.status(404).json({ error: '해당 도서가 없습니다.' });
      }
      res.json(results[0]);
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: '도서 삭제 실패' });
      }
      res.json({ success: true });
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: '도서 수정 실패' });
      }
      res.json({ success: true });
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, results) => {
      if (err) {
        ...
        return res.status(500).json({ error: '과일 조회 실패' });
      }
      if (results.length === 0) {
        return res.status(404).json({ error: '해당 과일이 없습니다.' });
      }
      res.json(results[0]);
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: '과일 삭제 실패' });
      }
      res.json({ success: true });
    }
  );
  ...
  connectionKdt.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.status(500).json({ error: '과일 수정 실패' });
      }
      res.json({ success: true });
    }
  );
      ...
      connectionKdt.query(prodSql, [product_id], (pErr, pRes) => {
        ...
        const insSql = `
          INSERT INTO green_cart
            (user_id, product_id, title, brand, kind, \`condition\`,
             shipping_fee, price, trade_type, region, image_main, added_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
        `;
        const params2 = [
